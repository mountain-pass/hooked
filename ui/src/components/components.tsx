...
export const BlackButton = ({ children, disabled, active, size = 'lg', className = '', onClick }: { disabled?: boolean, active?: boolean, size: 'sm' | 'md' | 'lg', onClick: () => void, className?: string, children: React.ReactNode }) => {
  ...
  return (
    <button
      ...
      ${active
          ? `border-neutral-500 bg-neutral-100 hover:bg-neutral-200 dark:bg-neutral-900 dark:bg-white/10 `
          : `border-neutral-300  bg-white hover:bg-neutral-200
          dark:text-neutral-400 dark:border-neutral-700 dark:bg-transparent dark:hover:bg-neutral-800 `} 
      px-3 py-2 
      ${dims} 
      text-sm 
      ${className}
...
export const Section = ({ visible, fade, naked, className = '', children }: { visible?: boolean, naked?: boolean, fade?: boolean, className?: string, children: React.ReactNode }) => (
  <section className={`
  rounded 
  ${naked ? '' : 'max-sm:p-2 sm:p-4 border border-neutral-300 dark:border-neutral-700 bg-slate-100 dark:bg-neutral-900'} 
  w-full flex flex-col gap-4 
  ${visible ? 'visible' : 'hidden'} 
  ${fade ? 'opacity-40 blur-sm' : ''} 
  ${className}
  `}>
    {children}
  </section >
...
/** Responsively naked on < sm screens. */
export const ResponsiveSection = ({ visible, fade, className = '', children }: { visible?: boolean, fade?: boolean, className?: string, children: React.ReactNode }) => (
  <section className={`rounded sm:p-4 sm:border sm:border-gray-200 sm:dark:border-neutral-700 sm:bg-slate-100 sm:dark:bg-neutral-900 w-full flex flex-col gap-4 ${visible ? 'visible' : 'hidden'} ${fade ? 'opacity-40 blur-sm' : ''} ${className}`}>
    {children}
  </section >
)