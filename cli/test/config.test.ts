...
describe('config', () => {
  ...
  it('a full envName should resolve #1', async () => {
    const config: Config = { env: { dogs: { foo: 'fido' }, cats: { foo: 'ginger' } }, scripts: {} }
    const [env, stdin, envName] = await resolveEnv(config, 'dogs', {}) // <- full envname
    expect(env).to.eql({ foo: 'fido' })
    expect(stdin).to.eql({})
    expect(envName).to.eql('dogs')
  })

  it('a full envName should resolve #2', async () => {
    const config: Config = { env: { default: { foo: 'fido' }, cats: { foo: 'ginger' } }, scripts: {} }
    const [env, stdin, envName] = await resolveEnv(config, 'cats', {}) // <- full envname
    expect(env).to.eql({ foo: 'ginger' })
    expect(stdin).to.eql({})
    expect(envName).to.eql('cats')
  })

  it('a partial envName should resolve', async () => {
    const config: Config = { env: { default: { foo: 'bar' } }, scripts: {} }
    const [env, stdin, envName] = await resolveEnv(config, 'def', {}) // <- partial name
    expect(env).to.eql({ foo: 'bar' })
    expect(stdin).to.eql({})
    expect(envName).to.eql('default')
  })

  it('an unknown envName should throw an error', async () => {
    const config: Config = { env: { default: { foo: 'bar' } }, scripts: {} }
  ...
  it('if not required, stdin placeholders should be ignored', async () => {
    ...
    const [env, stdin, envName] = await resolveEnv(config, 'default', { aaa: '111' })
  ...
  it('defined env values should compliment and override global envs', async () => {
    const config: Config = { env: { default: { bbb: 'xxx', ddd: 'zzz' } }, scripts: {} }
    const [env, stdin, envName] = await resolveEnv(config, 'default', {}, { aaa: '111', bbb: '222', ccc: '333' })
  ...
  it('env should resolve "string"', async () => {
    ...
    const [env, stdin, envName] = await resolveEnv(config, 'default', {})
  ...
  it('env should resolve $env', async () => {
    ...
    const [env, stdin, envName] = await resolveEnv(config, 'default', {}, { USER: 'fred' })
  ...
  it('env should resolve $stdin from provided inputs', async () => {
    ...
    const config: Config = { env: { default: { name: { $stdin: 'what is your name?' } } }, scripts: { } }
    const [env, stdin, envName] = await resolveEnv(config, 'default', { name: 'bob' })
